...
- 目標平台：Flowise + 現有前端 + 現有 FHIR API
- 目標效果：保留 Dify 版本的命令切換、風險分流、follow-up、六條模式、Mission/Option 檢索、summary/FHIR 長鏈

## 2. Repo 內交付內容
- Flowise 狀態結構：[FLOWISE_STATE_SCHEMA.json](C:/Users/閻星澄/Desktop/FHIR-main/FHIR-main/flowise/FLOWISE_STATE_SCHEMA.json)
- Dify 節點到 Flowise 節點映射：[FLOWISE_DIFY_NODE_MAP.json](C:/Users/閻星澄/Desktop/FHIR-main/FHIR-main/flowise/FLOWISE_DIFY_NODE_MAP.json)
- Flowise 藍圖：[AI_Companion_Flowise_Blueprint.json](C:/Users/閻星澄/Desktop/FHIR-main/FHIR-main/flowise/flows/AI_Companion_Flowise_Blueprint.json)
...
- Flowise client：[app/flowiseChatClient.js](C:/Users/閻星澄/Desktop/FHIR-main/FHIR-main/app/flowiseChatClient.js)
...
2. Node proxy 轉發到 Flowise Prediction API
3. Node proxy 保持 `/api/fhir/bundle`
4. 前端不直接暴露 Flowise API key

### 3.2 Flowise 狀態
- Flowise 的 `sessionId` 由 Node proxy 傳入，對應現有 `conversation_id/user`
...
- Mission / Option 共用同一份來源 PDF，但在 Flowise 內要拆成不同 retriever flow 或不同 prompt 鏈
...
- Flowise 版規劃：
  ...
  - 匯入 Flowise Document Store
  - 使用本地向量庫持久化
  - 由不同的 audit / answer prompt 控制知識使用方式

## 6. API 介面
### 6.1 Node -> Flowise
- Endpoint: `POST {FLOWISE_API_BASE_URL}/api/v1/prediction/{FLOWISE_CHATFLOW_ID}`
- Request body:
  - `question`
  - `streaming`
  ...
  - `overrideConfig.vars`
- Header:
  - `Authorization: Bearer {FLOWISE_API_KEY}`，若 Flowise 未啟用 flow auth 可省略
...
2. 用 [tools/build_flowise_assets.js](C:/Users/閻星澄/Desktop/FHIR-main/FHIR-main/tools/build_flowise_assets.js) 生成：
   - node map
   - state schema
   - prompts
   - flow blueprint
3. 設定 Flowise base URL / chatflow id / key
...
- Flowise health config 在 `/health` 可看到 `flowise_configured`
- `你好` 可回覆
- `我想死` 能進安全回應
...
## 9. 已知限制
- Flowise 官方 export/import JSON 會隨版本差異而變動；repo 目前保存的是可重建藍圖與完整 prompt/state 資產，而不是鎖死某一版 UI export。
- Dify 的 assigner 與 question-classifier 是平台專有 UI 概念；Flowise 版本會改成 JS function、變數節點與 prompt chain 組合。
- 若要做到完全圖形化 1:1 視覺流程，需要依實際 Flowise 安裝版本在 UI 內完成最後的節點拼裝。
