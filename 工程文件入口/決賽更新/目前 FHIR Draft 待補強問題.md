...
## 2026-05-01 近期迭代時曾遇到的待補強問題

> 註：以下這些問題，很多在 `2026-05-01` 晚上前後已陸續修掉，但它們仍然應該被保留在「待補強問題」裡，因為它們確實是最近兩三天 FHIR draft 迭代過程中碰到的真實缺口，而不是事後憑空整理出來的。

### 1. FHIR 欄位 mapping 與 TW Core 對照不夠直觀

#### 當時遇到的問題
1. 在導入 `TW IG Core` 時，哪些欄位該放 `Observation`、哪些該落 `QuestionnaireResponse`，實作時仍容易混淆。
2. 現場若沒有欄位對照表或轉換提示，展示時很難快速說清楚「這個欄位為什麼落在這裡」。
3. draft 雖然能產生 bundle，但 mapping decision 的說服力還不夠可視化。

#### 為什麼這也該算待補強問題
1. 這不是純 UI 問題，而是 FHIR draft 能不能被人理解的問題。
2. 如果評審或導師追問 mapping logic，而系統沒有輔助表或轉換按鈕，就很容易顯得規則只存在你腦中。

### 2. ClinicalImpression 與 HAPI 測試伺服器相容性仍會踩雷

#### 當時遇到的問題
1. `ClinicalImpression` 雖然語意上已經接近可展示版本，但在 HAPI FHIR 測試環境仍曾出現相容性問題。
2. 這代表問題不只在內容品質，也在於「能不能真的穩定送出去」。

#### 為什麼這也該算待補強問題
1. 對決賽展示來說，FHIR draft 若無法穩定送出，再漂亮的語意整理都會失分。
2. `ClinicalImpression` 本來就是 bundle 裡最敏感、最容易出現語意與格式雙重問題的資源。

### 3. report bundle 刷新後不一定能穩定保留

#### 當時遇到的問題
1. draft 當下生成成功，不代表刷新後還能順利取回。
2. 若 report bundle 沒有被穩定保存在 session cache，醫師端或 history trace 很容易斷鏈。
3. 使用者會誤以為「系統剛剛明明有產生」，但下一次進來卻像沒發生過。

#### 為什麼這也該算待補強問題
1. 這已經不只是內容問題，而是交付穩定性問題。
2. FHIR draft 若不能跨刷新保留，就很難說自己是一條可靠的資料交付流程。

### 4. FHIR history / bootstrap 流程不穩定

#### 當時遇到的問題
1. 最近兩三天的另一個明顯缺口，是 FHIR history 在 bootstrap 階段曾發生 crash。
2. 一旦 history 初始化失敗，前面辛苦整理出的 draft、trace 與回顧入口都會一起變得不可信。
3. 這類問題雖然不屬於單一資源欄位錯誤，但會直接破壞整條 FHIR draft 的可追溯性。

#### 為什麼這也該算待補強問題
1. 如果 FHIR history 一直不穩，評審很容易認為系統只有單次生成、沒有真正的追蹤能力。
2. 這也是最近兩三天你最實際碰到的工程風險之一，不能因為最後修掉了，就從歷程裡消失。

### 5. 最近這批問題的共同本質

1. 4/24 前後的問題，重點比較偏資源內容乾不乾淨、語氣有沒有過滿、FHIR 結構是否合理。
2. 5/1 這批新問題，則更偏向「交付穩定性、traceability、history 可回溯性、mapping 可解釋性」。
3. 換句話說，最近兩三天遇到的待補強問題，代表你關注的焦點已經從「能不能產生 draft」進一步進到「產生之後能不能穩定保存、穩定回看、穩定送出、穩定解釋」。
