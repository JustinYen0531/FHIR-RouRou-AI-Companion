...
這份文件不是單純抄錄討論紀錄，也不是只列外部資料。  
它的目的，是把「導師提醒了什麼」、「你怎麼理解成設計原則」、「最後又怎麼真的做進專案」完整扣在一起。
...
<sub>這張圖保留的是你和導師針對臨床使用情境、欄位形式與操作負擔的實際往返。它適合放在完整版前段，因為這份文件的核心不是抽象理念，而是這些提醒如何影響你後面的整體設計。</sub>

### 2. 導師確認 Observation 放法與 PoC 邊界

![導師確認 Observation 放法與 PoC 邊界](./導師互動與實作對照_附件/02_導師確認Observation畫面.png)

<sub>這張圖補到這裡，是因為它把一個很關鍵的設計判斷講得很直白：若是 AI 從對話中抓出的不舒服、嚴重程度、持續時間，應該先在前端顯示、請病人確認，再以上傳 `Observation` 的方式處理；而在比賽或 PoC 階段，FHIR 實作也不需要一開始就追到完整 profile compliance，只要 resource 結構正確即可。這段互動等於直接替你後來的「Observation 優先、病人確認先行、PoC 先求結構正確」提供了導師端背書。</sub>
